  ...
  return (
      ...
      <div className="p-3 md:p-6 space-y-3 md:space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-6">
          {/* Thông tin cá nhân */}
          <div className="lg:col-span-2 bg-white rounded-xl border border-[#E5E7EB] shadow-sm p-3 md:p-6 flex flex-col items-center text-center space-y-3 md:space-y-4">
            {/* Avatar */}
            ...
            {/* Tên & chức vụ */}
            <div>
              <h2 className="text-lg md:text-xl font-semibold text-[#1F2937]">{profile?.full_name ?? '—'}</h2>
              <p className="text-[#6B7280] text-xs md:text-sm">{profile?.position ?? '—'}</p>
              <div className="flex items-center justify-center mt-1 gap-2 text-xs md:text-sm text-[#16A34A]">
            ...
            {/* Thông tin chi tiết */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 md:gap-x-10 gap-y-2 md:gap-y-3 text-xs md:text-sm text-[#374151] mt-3 md:mt-4">
              <div>
                <span className="font-medium text-[#6B7280]">Mã nhân viên:</span>
                <div className="font-semibold text-xs md:text-sm">{profile?.employee_code ?? '—'}</div>
              </div>
              <div>
                <span className="font-medium text-[#6B7280]">Ngày vào làm:</span>
                <div className="text-xs md:text-sm">{profile?.start_date ? new Date(profile.start_date).toLocaleDateString('vi-VN') : '—'}</div>
              </div>
              <div>
                <span className="font-medium text-[#6B7280]">Email:</span>
                <div className="flex items-center justify-center gap-1 text-xs md:text-sm">
                  <Mail className="w-3 md:w-4 h-3 md:h-4 text-[#2563EB]" />
                  <span>{profile?.email ?? '—'}</span>
                </div>
              </div>
              <div>
                <span className="font-medium text-[#6B7280]">Số điện thoại:</span>
                <div className="text-xs md:text-sm">{profile?.phone ?? '—'}</div>
              </div>
              <div>
                <span className="font-medium text-[#6B7280]">Phòng ban:</span>
                <div className="flex items-center justify-center gap-1 text-xs md:text-sm">
                  <Briefcase className="w-3 md:w-4 h-3 md:h-4 text-[#2563EB]" />
                  <span>{profile?.department_name ?? '—'}</span>
                </div>
              </div>
              <div>
                <span className="font-medium text-[#6B7280]">Địa chỉ:</span>
                <div className="text-xs md:text-sm">{profile?.address ?? '—'}</div>
              </div>
              <div>
                <span className="font-medium text-[#6B7280]">Ngày sinh:</span>
                <div className="text-xs md:text-sm">{profile?.date_of_birth ? new Date(profile.date_of_birth).toLocaleDateString('vi-VN') : '—'}</div>
              </div>
            </div>

            {/* Nút hành động */}
              <div className="flex flex-col sm:flex-row gap-2 md:gap-4 mt-4 md:mt-6 w-full sm:w-auto justify-center">
              <button onClick={handleOpenEdit} disabled={loading} className={`flex items-center justify-center gap-2 px-3 md:px-5 py-2 rounded-lg text-xs md:text-sm font-medium whitespace-nowrap ${loading ? 'bg-[#93C5FD] text-white cursor-not-allowed' : 'bg-[#2563EB] text-white hover:bg-blue-700 transition-colors'}`}>
                <Edit className="w-3 md:w-4 h-3 md:h-4" />
                <span>Chỉnh sửa</span>
              </button>
              <button onClick={() => setShowChangePassword(true)} disabled={loading} className={`flex items-center justify-center gap-2 px-3 md:px-5 py-2 rounded-lg text-xs md:text-sm font-medium whitespace-nowrap ${loading ? 'border-[#93C5FD] text-[#93C5FD] cursor-not-allowed' : 'border border-[#2563EB] text-[#2563EB] hover:bg-blue-50 transition-colors'}`}>
                <KeyRound className="w-3 md:w-4 h-3 md:h-4" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 md:p-0">
          <div className="relative w-full max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 md:p-8 space-y-4 md:space-y-6 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
                  ...
                  <div>
                    ...
                    <input
                      className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-xs md:text-sm"
                  ...
                  {/* Email - chỉ hiển thị */}
                  <div>
                    <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-2">Email công ty</label>
                    <input
                      className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-600 cursor-not-allowed text-xs md:text-sm"
                      value={profile?.email || ''}
                      disabled
                    />
                  </div>
                  ...
                  <div>
                    ...
                    {profile?.department_name ? (
                      <input
                        className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-700 cursor-not-allowed text-xs md:text-sm"
                  ...
                  {/* Số điện thoại */}
                  <div>
                    <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-2">Số điện thoại</label>
                    <input
                      className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-xs md:text-sm"
                  ...
                  {/* Ngày sinh */}
                  <div>
                    <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-2">Ngày sinh</label>
                    <input
                      type="date"
                      className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-xs md:text-sm"
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-2">Địa chỉ</label>
                    <input
                      className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-xs md:text-sm"
              ...
              {/* Footer */}
              <div className="bg-gray-50 px-3 md:px-8 py-3 md:py-5 flex flex-col sm:flex-row justify-end gap-2 md:gap-4 border-t">
                <button
                  onClick={() => setShowEditModal(false)}
                  className="px-4 md:px-6 py-2 md:py-3 rounded-xl border border-gray-300 font-medium text-gray-700 hover:bg-gray-100 transition-colors text-xs md:text-sm order-2 sm:order-1"
                >
                  Hủy bỏ
                </button>
                <button
                  ...
                  className="px-4 md:px-8 py-2 md:py-3 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-xs md:text-sm order-1 sm:order-2"
                >
                  {loading ? (
                    <>Đang lưu...</>
                  ) : (
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              <div className="p-4 md:p-8 space-y-4 md:space-y-6">
                <div className="space-y-3 md:space-y-5">
                  <div>
                    ...
                    <input
                      type="password"
                      className="w-full px-2 md:px-4 py-2 md:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all outline-none text-xs md:text-sm"
                      placeholder="••••••••"
              ...
              <div className="bg-gray-50 px-3 md:px-8 py-3 md:py-5 flex flex-col sm:flex-row justify-end gap-2 md:gap-4 border-t">
                ...
                <button
                  ...
                  className="px-4 md:px-8 py-2 md:py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold hover:from-emerald-700 hover:to-teal-700 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-xs md:text-sm order-1 sm:order-2"
                >
                  {loading ? (
                    <>
                      <svg className="w-4 md:w-5 h-4 md:h-5 animate-spin" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" strokeLinecap="round" /></svg>
                      <span className="hidden md:inline">Đang xử lý...</span>
                      <span className="md:hidden">Xử lý...</span>
                    </>
                  ) : (
                    <>
                      <CheckCircle2 className="w-4 md:w-5 h-4 md:h-5" />
                      <span className="hidden md:inline">Xác nhận đổi mật khẩu</span>
